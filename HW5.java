...
public class HW5 {
	...
	private static double[] evaluate(Label[] yTrue, Label[] yPred) {
		...
		double total = yPred.length;
		
		for(int i = 0; i < yPred.length; i++){
			System.out.print(i+1 + "." + " " + "Actual: " + yTrue[i] + " - " + "Classified: ");
			System.out.println(yPred[i]);
			if(yPred[i].equals(yTrue[i])){
				correct++;
			}
			if(yPred[i].equals(Label.Positive) && yTrue[i].equals(Label.Positive)){
				truepositive++;
			}
			if(yPred[i].equals(Label.Positive) && yTrue[i].equals(Label.Negative)){
				falsepositive++;
			}
			if(yPred[i].equals(Label.Negative) && yTrue[i].equals(Label.Positive)){
				falsenegative++;
			}
		}
		
		
		scores[0] = correct/total;
		scores[1] = truepositive/(truepositive + falsepositive);
		scores[2] = truepositive/(truepositive + falsenegative);
		// TO DO
		return scores;
	}
